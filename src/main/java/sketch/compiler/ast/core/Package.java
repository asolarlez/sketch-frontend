...
/**
 * Container class containing all of the state for a Sketch stream type. A
 * <code>Package</code> may or may not have a name; if there is no name, this is an
 * anonymous stream. It also has a type (as an integer), a stream type (with I/O data
 * types), a parameter list, a list of variable declarations (as <code>Statement</code>s;
 * they should all actually be <code>StmtVarDecl</code>s), and a list of function
 * declarations (as <code>Function</code> objects). The stream type may be
 * <code>null</code>, in which case the compiler will need to determine the stream type on
 * its own.
 * 
