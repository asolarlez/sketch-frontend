...
public StreamItParserFE(ParserSharedInputState state) {
  ...
  tokenNames = _tokenNames;
	...
	public final StreamSpec  filter_body(
		...
		try {      // for error handling
			...
			do {
				...
				default:
					boolean synPredMatched1766 = false;
					if (((_tokenSet_0.member(LA(1))))) {
						int _m1766 = mark();
						synPredMatched1766 = true;
						inputState.guessing++;
						try {
							{
							data_type();
							...
							synPredMatched1766 = false;
						}
						rewind(_m1766);
						inputState.guessing--;
					}
					if ( synPredMatched1766 ) {
						fn=function_decl();
						if ( inputState.guessing==0 ) {
							funcs.add(fn);
						}
	...
	public final Statement  expr_statement() throws RecognitionException, TokenStreamException {
		...
		try {      // for error handling
			boolean synPredMatched1845 = false;
			if (((LA(1)==INCREMENT||LA(1)==DECREMENT||LA(1)==ID))) {
				int _m1845 = mark();
				synPredMatched1845 = true;
				inputState.guessing++;
				try {
					{
					incOrDec();
					...
					synPredMatched1845 = false;
				}
				rewind(_m1845);
				inputState.guessing--;
			}
			if ( synPredMatched1845 ) {
				x=incOrDec();
				if ( inputState.guessing==0 ) {
					s = new StmtExpr(x);
				}
				...
				boolean synPredMatched1848 = false;
				if (((LA(1)==ID))) {
					int _m1848 = mark();
					synPredMatched1848 = true;
					inputState.guessing++;
					try {
						{
						left_expr();
						...
						synPredMatched1848 = false;
					}
					rewind(_m1848);
					inputState.guessing--;
				}
				if ( synPredMatched1848 ) {
					s=assign_expr();
				}
				else {
					boolean synPredMatched1850 = false;
					if (((LA(1)==ID))) {
						int _m1850 = mark();
						synPredMatched1850 = true;
						inputState.guessing++;
						try {
							{
							match(ID);
							...
							synPredMatched1850 = false;
						}
						rewind(_m1850);
						inputState.guessing--;
					}
					if ( synPredMatched1850 ) {
						x=func_call();
						if ( inputState.guessing==0 ) {
							s = new StmtExpr(x);
						}
	...
	public final Expression  castExpr() throws RecognitionException, TokenStreamException {
		...
		try {      // for error handling
			boolean synPredMatched1895 = false;
			if (((LA(1)==LPAREN))) {
				int _m1895 = mark();
				synPredMatched1895 = true;
				inputState.guessing++;
				try {
					{
					match(LPAREN);
					...
					synPredMatched1895 = false;
				}
				rewind(_m1895);
				inputState.guessing--;
			}
			if ( synPredMatched1895 ) {
				{
				l = LT(1);
				match(LPAREN);
				t=primitive_type();
