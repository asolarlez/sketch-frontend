...
for tr in 1 2 3 4 5; do
	for i in `ls ./model_usage`; do
		cd ./model_usage/$i
		$ms --slv-timeout 2 model_old_cegis.sk -V 10 &> "../../outputs/$i.model_old_cegis_trial$tr.txt";
		itr=`grep 'GOT THE CORRECT' "../../outputs/$i.model_old_cegis_trial$tr.txt" | awk '{print $6}'`;
		time=`grep 'Total time' "../../outputs/$i.model_old_cegis_trial$tr.txt" | awk '{print $4}'`;
