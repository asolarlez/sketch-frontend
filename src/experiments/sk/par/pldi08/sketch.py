...
class Test:
    def __init__ (self, file, workdir, logfile, cmd):
        ...
        assert os.access (path, os.R_OK), '%s not there'% (path)
    ...
    def run (self):
        ...
        (open (self.name +'.out', 'w')).write (self.out)
        (open (self.name +'.err', 'w')).write (self.err)
...
if __name__ == '__main__':
    ...
    try:
        tests = (test ('benchmarks/regtest/miniTest1.sk', extraopts=o),
                 test ('benchmarks/regtest/miniTest16.sk', extraopts=o))
