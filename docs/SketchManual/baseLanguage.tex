...
p3.x = px.x; p3.y = p2.y;
\end{lstlisting}
Such an assignment requires that \C{p2} not be \C{null}. Similarly, an assignment from a temporary structure to a heap allocated structure is also interpreted as a field-by-field copy, with a similar assumption that the reference will not be null. Failure to satisfy the assumption will cause an exception. 
...
\subsection{Algebraic Datatypes}\seclabel{padding}
